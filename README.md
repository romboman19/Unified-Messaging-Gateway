...
Portainer розгортає **один** compose-файл, а стек нормально складається з двох
(базовий плюс прод-оверрайд із тегами злиття `!reset`/`!override`). Для цього в
репозиторії лежить уже зведений `docker-compose.portainer.yml`.

1. Stacks → Add stack → **Repository**.
2. URL — `https://github.com/romboman19/Unified-Messaging-Gateway`,
   reference `refs/heads/main`.
3. Compose path — `docker-compose.portainer.yml`.
4. У розділі Environment variables задати ті самі змінні, що в
   [.env.example](.env.example). Секрети у файл не зашиті — там лише
   плейсхолдери.

   Два підводні камені саме тут. Portainer **не розгортає** `${ІНША_ЗМІННА}`
   всередині значення змінної — пишіть готові значення, а не посилання. І
   `DBSMS_WEBHOOK_SECRET` потрапляє в query-рядок, де `+` декодується як
   пробіл, тому секрет має бути URL-безпечним (`openssl rand -base64 24 | tr
   '+/' '-_'`).
5. Deploy. Перше розгортання довше звичайного: три образи збираються з
   вихідників, а образ SMS-сервера ще й тягне вендорський архів.

На хості мають бути вільні порти `8083` (веб), `8084` (інтерфейс SMS-сервера)
і `44444/udp` (реєстрація GSM-шлюза).

Поле **Compose path** має містити рівно `docker-compose.portainer.yml` — зайвий
пробіл на початку Portainer сприймає як частину імені файлу і повідомляє, що
файл не знайдено.

Файл згенерований, не редагуйте його вручну — після зміни будь-якого з двох
вихідних перезберіть командою з шапки самого файлу.
...
Перевірка автоматична — раз на добу о 06:00 для кожної увімкненої лінії.
Результат зберігається на ендпоінті, а якщо баланс нижчий за поріг, підіймається
алерт: `warning` при низькому залишку і `critical` при заборгованості.

Код USSD задається **на кожній лінії**, бо різниться не лише за оператором, а й
за тарифом:
...
```json
// configJson ендпоінта
{ "balanceUssd": "*111#", "lowBalanceThreshold": 20 }
```

Без коду лінія просто пропускається — не кожна SIM передплачена.

**Знак суми передається словом, а не мінусом.** Контрактні номери відповідають
`zaborhovanist 48.17 hrn` (заборгованість) або `avans 101.83 hrn` (аванс) —
обидва числа додатні. Прочитати перше число як є означало б показати борг у
48 гривень як 48 гривень на рахунку, тобто рівно навпаки. Тому знак визначається
формулюванням. З тієї ж причини сума розпізнається лише з копійками або поруч із
назвою валюти: інакше довгий номер особового рахунку в тій самій відповіді
сприймався б за баланс.
