...
import { Trash2, Plus, Radio, Power, PowerOff, AlertCircle, Info } from 'lucide-react';
...
const REQUIRES_REAL_ADAPTER: Account['type'][] = ['sms', 'whatsapp', 'signal'];
...
export default function ChannelsPage() {
  ...
  const [newEndpoint, setNewEndpoint] = useState<
    Record<string, { label: string; externalId: string; phone: string; enabled: boolean }>
  >({});
  ...
  async function createEndpoint(accountId: string) {
    ...
    setNewEndpoint((prev) => ({
      ...prev,
      [accountId]: { label: '', externalId: '', phone: '', enabled: true },
    }));
    await fetchAccounts();
  }
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-4 mt-4 flex items-start gap-3 rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
        ...
        <div>
          <b>Milestone 1 підтримує лише Mock-канал.</b>{' '}
          Реальні SMS/WhatsApp/Signal ще не підключені — це Milestones 2/4/5.
          Якщо створити акаунт Signal/WhatsApp/SMS, він отримає статус «не підключено»,
          бо потрібен справжній адаптер: GoIP-сервер, UnoAPI/WhatsApp Business API
          або signal-cli з QR-кодом зв'язування пристрою.
        </div>
      </div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setNewAccount((p) => ({
                  ...
                  type: t,
                  adapter: t,
                  status: REQUIRES_REAL_ADAPTER.includes(t) ? 'inactive' : 'active',
                }));
              }}
            >
      ...
      <section className="mt-6 space-y-4">
        ...
        {accounts.map((acc) => {
          const isPlaceholder = REQUIRES_REAL_ADAPTER.includes(acc.type);
            ...
            <div key={acc.id} className="rounded-lg bg-white p-4 shadow">
              <div className="flex items-start justify-between">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-slate-500">
                      ...
                      <span className={isActive ? 'text-green-600' : 'text-slate-400'}>
                        {isActive ? 'активний' : 'не підключено'}
                      </span>
                      {isPlaceholder && (
                        <span className="ml-2 inline-flex items-center gap-1 rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-700">
                          <AlertCircle size={12} />
                          потребує реального адаптера
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    title={isActive ? 'Вимкнути' : 'Увімкнути'}
                  >
                    ...
                    {acc.endpoints.map((ep) => (
                      ...
                      >
                        <div className="text-sm">
                          {ep.label}
                          {ep.externalId && (
                            <span className="ml-2 text-slate-400">{ep.externalId}</span>
                          )}
                          {ep.phoneE164 && (
                          ...
                          {isPlaceholder && (
                            <span className="ml-2 text-xs text-amber-600">
                              (endpoint не працює без реального адаптера)
                            </span>
                          )}
                        </div>
                        <div className="flex gap-2">
              ...
              >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewEndpoint((prev) => ({
                        ...prev,
                        [acc.id]: {
                          ...(prev[acc.id] || { externalId: '', phone: '', enabled: true }),
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewEndpoint((prev) => ({
                        ...prev,
                        [acc.id]: {
                          ...(prev[acc.id] || { label: '', phone: '', enabled: true }),
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewEndpoint((prev) => ({
                        ...prev,
                        [acc.id]: {
                          ...(prev[acc.id] || { label: '', externalId: '', enabled: true }),
                ...
                <div className="md:col-span-1 flex items-center">
                  <button
                    type="submit"
                    className="flex w-full items-center justify-center gap-1 rounded border py-2 hover:bg-slate-100"
                  >
